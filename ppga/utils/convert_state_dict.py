...
# 0. define measurable aspects like average foot height, speed, energy-efficiency, etc.

def avg_foot_height(traj):
    """
    Given trajectory, calculate the average foot height of each step
    
    Input:
        traj - a sequence of states and additional information that encode the foot height
    Output:
        output - the average foot height
    """
    pass

# 1. Our eval loop. We want to compare two policies (or a batch of pairs) and use our measurable functions
# to determine which policy is better

# need DataLoader here
num_evals = 10
for i in range(num_evals):
    # a. Query a batch of policy pairs from our dataloader: shape (batch_size, 2 policies)

    # b. Get a batch of trajectory pairs from our policies: shape (batch_size, 2 trajectories)

    # c. Compute the score for each measurable function: shape (num_functions, batch_size, 2 trajectory scores)

    # d. Collapse the matrix to get binary score on which policy is superior: shape (num_functions, batch_size, 1 binary score)

    pass

# print the collected binary scores.