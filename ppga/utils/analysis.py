...
def compile_cdf(cfg, dataframes=None):
    ...
    x = np.linspace(cfg.objective_range[0], cfg.objective_range[1],
                    cfg.objective_resolution)
    all_y_vals = []
    ...
    cdf = pd.DataFrame(all_data.T,
                       columns=[
                           'Objective', 'Threshold Percentage (Mean)',
...
def get_results_dataframe(env_name: str,
                          algorithm: str,
                          keywords: list[str],
                          name=None,
                          scaling_exp=False):
    ...
    if algorithm in list1:
        if scaling_exp:
            keys = [
                'QD/iteration', 'QD/coverage (%)', 'QD/QD Score',
                'QD/Best Score', 'QD/average performance'
            ]
        else:
            keys = [
                'QD/iteration', 'QD/coverage (%)', 'QD/QD Score',
    ...
    for run in runs:
        ...
        if res:
            if algorithm in list1:
                ...
                for key in keys:
                    cached_data_path = cache_dir.joinpath(
                        Path(f'{run.storage_id}_{key}.csv'))
                    if cached_data_path.exists():
                        print(
                            f'Loading cached data for run {run.name} and {key=} from cache'
                        )
                        df = pd.read_csv(str(cached_data_path))
                    else:
                        df = pd.DataFrame(
                            run.scan_history(keys=['iteration'] + [key]))
                ...
                if scaling_exp:
                    hist = pd.DataFrame(data=hist, columns=keys).rename(
                        columns={
...
def get_pgame_df(exp_dir, reevaluated_archive=False, save=False):
    ...
    for seed in seeds:
        subdir = sorted(
            glob.glob(exp_dir + '/' + f'*{seed}*/checkpoints/checkpoint_*'))[0]
...
def get_ppga_df(exp_dir, reevaluated_archive=False):
    ...
    dataframes = []
    for seed in seeds:
        subdir = sorted(glob.glob(exp_dir + '/' +
                                  f'*{seed}*/checkpoints/cp_*'))[
                                      -1]  # gets the most recent checkpoint
        if reevaluated_archive:
...
def plot_cdf_data(algorithm: str,
                  alg_data_dirs: dict,
                  archive_type: str,
                  reevaluated_archives=False,
                  axs=None,
    ...
    for i, (exp_name, env_dir) in enumerate(alg_data_dirs.items()):
        base_cfg = Box(shared_params[exp_name])
...
def load_and_eval_pgame_archive(exp_name, exp_dirs, seed, data_is_saved=False):
    ...
    if data_is_saved:
        ...
        new_archive_fp = os.path.join(save_path,
                                      f'{exp_name}_reeval_archive.pkl')
...
def load_and_eval_ppga_archive(exp_name, exp_dirs, seed, data_is_saved=False):
    exp_dir = exp_dirs[exp_name]
    cp_path = sorted(glob.glob(exp_dir + '/' + f'*{seed}*/checkpoints/cp_*'))[
        -1]  # gets the most recent checkpoint
    save_path = cp_path
    if not os.path.exists(save_path):
        os.mkdir(save_path)
...
def visualize_reevaluated_archives():
    ...
    for i, exp_name in enumerate(PGAME_DIRS.keys()):
        _, new_pgame_archive = load_and_eval_pgame_archive(exp_name,
                                                           seed,
                                                           data_is_saved=True)
        _, new_ppga_archive = load_and_eval_ppga_archive(exp_name,
                                                         seed,
                                                         data_is_saved=True)
...
def print_corrected_qd_metrics(algorithm: str, exp_dirs, algorithm_type: str):
    ...
    for exp_name in exp_dirs.keys():
        ...
        for seed in seeds:
            ...
            _, new_archive = eval_fn(exp_name,
                                     exp_dirs,
                                     seed,
                                     data_is_saved=False)
...
def get_hyperparam_gridsearch_results():
    ...
    for run in runs:
        if 'gradsteps' in run.name or 'walksteps' in run.name or 'v2_clipped_nonadaptive' in run.name:
            ...
            hist[
                'name'] = f'(N1, N2) = ({run.config["calc_gradient_iters"]}, {run.config["move_mean_iters"]})'
            hist_list.append(hist)
...
def plot_qd_results_main():
    ...
    fig, axs = plt.subplots(4,
                            4,
                            figsize=(16, 12),
                            subplot_kw=dict(projection='data_post_processor'))

    for j, env in enumerate(envs):
        all_data = []
        for algorithm in algorithms.keys():
            df = get_results_dataframe(
                env, algorithm, keywords=algorithms[algorithm]['keywords'])
        ...
        all_data = pd.concat(all_data,
                             ignore_index=True).sort_values(by=['Num Evals'])

        ax_best = sns.lineplot(x='Num Evals',
                               y='QD/best score',
                               errorbar='sd',
                               data=all_data,
                               ax=axs[0][envs.index(env)],
                               hue='name',
                               hue_order=alg_names,
                               legend=False)
        ax_qd = sns.lineplot(x='Num Evals',
                             y='QD/QD Score',
                             errorbar='sd',
                             data=all_data,
                             ax=axs[1][envs.index(env)],
                             hue='name',
                             hue_order=alg_names,
                             legend=False)
        ax_cov = sns.lineplot(x='Num Evals',
                              y="QD/coverage (%)",
                              errorbar='sd',
                              data=all_data,
                              hue='name',
                              ax=axs[2][envs.index(env)],
                              hue_order=alg_names,
                              legend=False)
    ...
    plot_cdf_data('QDPG',
                  ...
                  reevaluated_archives=False,
                  axs=axs)
    plot_cdf_data('SEP-CMA-MAE',
                  SEP_CMA_MAE_DIRS,
                  archive_type='pyribs',
                  reevaluated_archives=False,
                  axs=axs)
    plot_cdf_data('CMA-MAEGA(TD3, ES)',
                  CMA_MAEGA_TD3_ES_DIRS,
                  archive_type='pyribs',
                  reevaluated_archives=False,
                  axs=axs)
...
def n1_n2_plots():
    ...
    all_data = []
    all_data.append(
        get_results_dataframe('humanoid',
                              'PPGA',
                              keywords=['paper', 'v2_clipped_nonadaptive'],
                              name='(N1, N2) = (10, 10)'))  # baseline
    all_data.append(
        get_results_dataframe('humanoid',
                              'PPGA',
                              keywords=['gradsteps_10_walksteps_5'],
                              name='(N1, N2) = (10, 5)'))
    all_data.append(
        get_results_dataframe('humanoid',
                              'PPGA',
                              keywords=['gradsteps_5_walksteps_10'],
                              name='(N1, N2) = (5, 10)'))
    all_data.append(
        get_results_dataframe('humanoid',
                              'PPGA',
                              keywords=['gradsteps_1_walksteps_1'],
                              name='(N1, N2) = (1, 1)'))
    all_data.append(
        get_results_dataframe('humanoid',
                              'PPGA',
                              keywords=['gradsteps_5_walksteps_5'],
                              name='(N1, N2) = (5, 5)'))
    for df in all_data:
    ...
    all_data = pd.concat(all_data,
                         ignore_index=True).sort_values(by=['QD/iteration'])
...
def td3_ablation_plots():
    ...
    all_data = []
    ppga_df = get_results_dataframe(
        'humanoid', 'PPGA', keywords=['paper',
                                      'v2_clipped_nonadaptive'])  # baseline
...
def plot_scaling_experiment():
    ...
    ppga_df = get_results_dataframe('humanoid',
                                    'PPGA',
                                    keywords=['4k'],
                                    scaling_exp=True)
    evals = ppga_df['QD/iteration'] * algorithms['PPGA']['evals_per_iter']
    ...
    pgame_df = get_results_dataframe('humanoid',
                                     'PGA-ME',
                                     keywords=['24hr'],
                                     scaling_exp=True)
    ...
    all_data = pd.concat(
        [ppga_df, pgame_df],
        ignore_index=True).sort_values(by='Relative Time (Wall)')
